//
//  RecommendedProductCategoryListView.swift
//  Josimi
//
...
struct RecommendedProductCategoryListView: View {
    @Environment(\.presentationMode) var presentationMode
    @StateObject private var viewModel = RecommendedProductViewModel()
    var categoryName: String // 카테고리명을 받는 프로퍼티
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                Text("총 \(viewModel.productCategorys.filter { $0.productType == "After" }.count)개")
                    .font(.system(size: 14))
                Spacer()
            }
            .padding(.vertical, 10)
            ...
            ScrollView {
                LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 20), count: 4), spacing: 20) {
                    ForEach(viewModel.productCategorys.filter { $0.productType == "After" }) { product in
                        RecommendedProductCategoryNormalItemView(productCategory: product)
                            .foregroundColor(.black)
                    }
                }
                .padding(.horizontal, 20)
            ...
            VStack {
                ...
                .alert(isPresented: $showAlert) {
                    Alert(title: Text("성분요청 완료"), message: Text("성분분석 요청이 완료되었습니다. \n 분석이 완료되면 알림을 보내드릴께요."), dismissButton: .default(Text("확인").foregroundColor(.black)))
                }
        ...
        .onAppear {
            viewModel.fetchProductsByCategory(categoryName: categoryName) // 카테고리명에 맞는 데이터를 가져옵니다.
        }
    }
}

#Preview {
    RecommendedProductCategoryListView(categoryName: "쌀류") // 임의 카테고리명을 넣어 미리보기 테스트
}
